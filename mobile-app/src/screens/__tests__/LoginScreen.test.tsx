...
describe('LoginScreen（手動入力モード）', () => {
  it('タイトルと入力フォームが表示される', () => {
    const { getByText, getByPlaceholderText } = render(<LoginScreen onLogin={jest.fn()} />);
  ...
  it('フォーム入力後にログインが成功するとコールバックが呼ばれる', async () => {
    ...
    const { getByText, getByPlaceholderText } = render(<LoginScreen onLogin={onLogin} />);
...
describe('LoginScreen（プリセットモード）', () => {
  it('aws-outputs.json がある場合はボタンのみ表示', () => {
    ...
    const { getByText, queryByPlaceholderText } = render(<LoginScreen onLogin={jest.fn()} />);
