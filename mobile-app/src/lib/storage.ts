...
export function loadConfigFromOutputs(): AuthConfig | null {
  try {
    const stack =
      _rawOutputs['PoiWebhookStack'] ?? (Object.values(_rawOutputs)[0] as Record<string, string> | undefined);
...
export const Storage = {
  ...
  async getAuthConfig(): Promise<AuthConfig | null> {
    const pairs = await AsyncStorage.multiGet([KEYS.API_URL, KEYS.CLIENT_ID, KEYS.COGNITO_DOMAIN]);
  ...
  async getNotifySettings(): Promise<NotifySettings> {
    const pairs = await AsyncStorage.multiGet([KEYS.NOTIFY_EXPEDITION, KEYS.NOTIFY_REPAIR, KEYS.NOTIFY_CONSTRUCTION]);
    return {
