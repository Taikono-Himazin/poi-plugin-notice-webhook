...
describe('syncWidgetData', () => {
  it('タイマーデータを JSON 文字列でネイティブモジュールに渡す', async () => {
    const timers = [
      { type: 'expedition' as const, slot: 2, completesAt: '2026-01-01T12:00:00Z', message: '遠征完了' },
    ];
    const lastSync = 1700000000000;

    await syncWidgetData(timers, lastSync);
  ...
  it('ネイティブモジュールがエラーでも例外を投げない', async () => {
    ...
    await expect(syncWidgetData([], null)).resolves.toBeUndefined();
  });
});
