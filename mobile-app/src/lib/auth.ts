...
async function performOAuthLogin(config: AuthConfig, extraParams: Record<string, string> = {}): Promise<LoginResult> {
...
export async function refreshTokens(): Promise<string | null> {
  const [config, refreshToken] = await Promise.all([Storage.getAuthConfig(), Storage.getRefreshToken()]);
