...
describe('extractRegion', () => {
  test('API Gateway URL からリージョンを抽出する', () => {
    expect(extractRegion('https://abc123.execute-api.ap-northeast-1.amazonaws.com/v1')).toBe('ap-northeast-1');
  });

  test('us-east-1 リージョン', () => {
    expect(extractRegion('https://xyz.execute-api.us-east-1.amazonaws.com/prod')).toBe('us-east-1');
  });
...
describe('extractTimersFromBody', () => {
  ...
  describe('遠征 (expedition)', () => {
    ...
    test('期限切れの遠征はスキップする', () => {
      const body = {
        api_deck_port: [{ api_id: 2, api_mission: [1, 5, past, 0] }],
      };
      const timers = extractTimersFromBody('/kcsapi/api_port/port', body);
    ...
    test('getStoreFn で遠征名を取得する', () => {
      ...
      const body = {
        api_deck_port: [{ api_id: 2, api_mission: [1, 5, future, 0] }],
      };
  ...
  describe('入渠 (repair)', () => {
    ...
    test('ndock API から直接抽出する', () => {
      const body = [{ api_id: 1, api_ship_id: 50, api_complete_time: future }];
    ...
    test('getStoreFn で艦名を取得する', () => {
      ...
      const body = {
        api_ndock: [{ api_id: 1, api_ship_id: 100, api_complete_time: future }],
      };
      const timers = extractTimersFromBody('/kcsapi/api_port/port', body, getStore);
  ...
  describe('建造 (construction)', () => {
    ...
    test('kdock API から直接抽出する', () => {
      const body = [{ api_id: 1, api_ship_id: 300, api_complete_time: future }];
    ...
    test('getStoreFn で建造艦名を取得する', () => {
      ...
      const body = {
        api_kdock: [{ api_id: 1, api_ship_id: 300, api_complete_time: future }],
      };
      const timers = extractTimersFromBody('/kcsapi/api_port/port', body, getStore);
  ...
  describe('複合ケース', () => {
    test('遠征・入渠・建造を同時に抽出する', () => {
      const body = {
        api_deck_port: [{ api_id: 2, api_mission: [1, 5, future, 0] }],
        api_ndock: [{ api_id: 1, api_ship_id: 100, api_complete_time: future }],
        api_kdock: [{ api_id: 1, api_ship_id: 200, api_complete_time: future }],
      };
      const timers = extractTimersFromBody('/kcsapi/api_port/port', body);
