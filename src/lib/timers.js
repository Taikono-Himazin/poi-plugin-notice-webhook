...
function extractTimersFromBody(path, body, getStoreFn) {
  ...
  for (const dock of ndock) {
    ...
    try {
      ...
      if (shipInstance) {
        ...
        if (shipName) {
          const lv = shipInstance.api_lv ?? '';
          const nowHp = shipInstance.api_nowhp ?? '';
          const maxHp = shipInstance.api_maxhp ?? '';
          widgetMsg = `${shipName} Lv.${lv} HP ${nowHp}/${maxHp}`;
        }
      }
    } catch (_) {}
    ...
    timers.push({
      ...
      widgetMessage: widgetMsg,
    });
  }

  // 建造: kdock の api_ship_id !== 0 && !== -1, api_complete_time > 0
  const kdock =
    path === '/kcsapi/api_port/port' ? body.api_kdock || [] : path === '/kcsapi/api_get_member/kdock' ? body || [] : [];
  for (const dock of kdock) {
    ...
    try {
      ...
      if (shipName) {
        const isLarge = (dock.api_item1 || 0) >= 1000;
        const res = [dock.api_item1, dock.api_item2, dock.api_item3, dock.api_item4].map((v) => v ?? '?').join('/');
        widgetMsgC = `${shipName} ${isLarge ? '大型' : '通常'} ${res}`;
      }
    } catch (_) {}
