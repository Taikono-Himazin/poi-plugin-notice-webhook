...
describe('PUT /timers (sync)', () => {
  ...
  test('DeviceNotRegistered のトークンは自動削除する', async () => {
    let queryCallCount = 0
    ddbMock
      .on(GetCommand).resolves({ Item: { userId: 'u' } })
      .on(QueryCommand).callsFake(() => {
        queryCallCount++
        if (queryCallCount === 1) return { Items: [] }
        return { Items: [
          { userId: 'u', pushToken: 'ExponentPushToken[stale]' },
        ] }
      })
      .on(PutCommand).resolves({})
      .on(DeleteCommand).resolves({})
    ...
    const pushTokenDelete = deleteCalls.find(
      c => c.args[0].input.TableName === 'push-tokens'
    )
  ...
  test('サイレントプッシュの失敗はタイマー同期に影響しない', async () => {
    ddbMock
      .on(GetCommand).resolves({ Item: { userId: 'u' } })
      .on(QueryCommand).resolvesOnce({ Items: [] })
      .on(QueryCommand).resolvesOnce({ Items: [
        { userId: 'u', pushToken: 'ExponentPushToken[abc]' },
      ] })
      .on(PutCommand).resolves({})
