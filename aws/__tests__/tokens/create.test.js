...
describe('POST /tokens (create)', () => {
  ...
  test('トークンを正常に作成できる', async () => {
    ddbMock.on(QueryCommand).resolves({ Count: 0, Items: [] }).on(PutCommand).resolves({});
  ...
  test('free プランでトークン上限 (1) に達したら 403', async () => {
    ...
    ddbMock
      .on(GetCommand)
      .resolves({ Item: { userId: 'user-1', plan: 'free' } })
      .on(QueryCommand)
      .resolves({ Count: 1, Items: [{}] });
  ...
  test('paid プランなら上限 10 まで作成できる', async () => {
    ...
    ddbMock
      ...
      .on(QueryCommand)
      .resolves({ Count: 5, Items: Array(5).fill({}) })
      .on(PutCommand)
      .resolves({});
  ...
  test('paid プランでも上限 10 に達したら 403', async () => {
    ...
    ddbMock
      ...
      .on(QueryCommand)
      .resolves({ Count: 10, Items: Array(10).fill({}) });
