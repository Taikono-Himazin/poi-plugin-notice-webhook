...
// 期限切れ通知スケジュール名（決定論的）
function expiryScheduleName(userId, phase) {
  const hash = crypto.createHash('md5').update(userId).digest('hex').slice(0, 8)
  return `poi-expiry-${hash}-${phase}`
}

// ms タイムスタンプを EventBridge Scheduler の at() 式に変換
function toScheduleExpr(ms) {
  return `at(${new Date(ms).toISOString().replace(/\.\d{3}Z$/, '')})`
}

/**
 * 有料プランの期限切れ通知スケジュールを登録する。
 * 既存スケジュールがあれば削除してから再作成（再購入時の上書き）。
 */
async function scheduleExpiryNotifications(userId, paidUntil) {
  const now = Date.now()
  const phases = [
    { phase: '7d',     fireAt: paidUntil - 7 * 24 * 3600 * 1000 },
    { phase: '1d',     fireAt: paidUntil - 1 * 24 * 3600 * 1000 },
    { phase: 'expired', fireAt: paidUntil + 60 * 1000 },
  ]

  for (const { phase, fireAt } of phases) {
    const name = expiryScheduleName(userId, phase)

    // 既存スケジュールを削除（なければ無視）
    await scheduler.send(new DeleteScheduleCommand({ Name: name })).catch(() => {})

    // 発火時刻が過去の場合は作成しない（7日前・1日前通知が購入時点で既に過去の場合）
    if (fireAt <= now) {
      console.info(`[payjp/webhook] skipping past schedule phase=${phase} for userId=${userId}`)
      continue
    }

    await scheduler.send(new CreateScheduleCommand({
      Name:                       name,
      ScheduleExpression:         toScheduleExpr(fireAt),
      ScheduleExpressionTimezone: 'UTC',
      FlexibleTimeWindow:         { Mode: 'OFF' },
      Target: {
        Arn:     process.env.EXPIRY_NOTIFY_FUNCTION_ARN,
        RoleArn: process.env.EXPIRY_SCHEDULER_ROLE_ARN,
        Input:   JSON.stringify({ userId, phase, expectedPaidUntil: paidUntil }),
      },
      ActionAfterCompletion: 'DELETE',
    }))

    console.info(`[payjp/webhook] scheduled expiry phase=${phase} at ${new Date(fireAt).toISOString()} for userId=${userId}`)
  }
}
  ...
  try {
    switch (eventType) {
      ...
      case 'checkout.session.completed': {
        ...
        // 期限切れ前後の通知スケジュールを登録（env が未設定の場合はスキップ）
        if (process.env.EXPIRY_NOTIFY_FUNCTION_ARN && process.env.EXPIRY_SCHEDULER_ROLE_ARN) {
          await scheduleExpiryNotifications(userId, paidUntil)
        }
        break
      }
