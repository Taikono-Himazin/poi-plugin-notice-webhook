...
export class PoiWebhookStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    const subscribeFn = fn('SubscribeFunction', 'billing/subscribe.js')
    accountsTable.grantReadWriteData(subscribeFn)

    const cancelFn = fn('CancelFunction', 'billing/cancel.js')
    accountsTable.grantReadWriteData(cancelFn)
    ...
    billingRes.addResource('subscribe').addMethod('POST',      lam(subscribeFn),    withAuth)
    billingRes.addResource('subscription').addMethod('DELETE', lam(cancelFn),       withAuth)
    ...
    new CfnOutput(this, 'WebhooksEndpoint',      { value: `${api.url}webhooks` })
    new CfnOutput(this, 'TimersEndpoint',        { value: `${api.url}timers` })
    new CfnOutput(this, 'AccountConfigEndpoint', { value: `${api.url}account/config` })
